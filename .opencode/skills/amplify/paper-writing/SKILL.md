...
<HARD-GATE>
Do NOT begin paper writing until user explicitly requests it AND G4 checklist is fully satisfied. If G4 has not passed, invoke `results-integration` first. No exceptions.
</HARD-GATE>
...
**REQUIRED SUB-SKILL:** Follow `multi-round-deliberation` protocol (max 5 rounds).
...
**REQUIRED SUB-SKILL:** Invoke `figure-quality-standards` and run the per-figure checklist on every figure and table in the paper.
...
**REQUIRED SUB-SKILL:** Follow `multi-round-deliberation` protocol (max 5 rounds for full-paper).
