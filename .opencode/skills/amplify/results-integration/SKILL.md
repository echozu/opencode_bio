...
**REQUIRED SUB-SKILL:** Invoke `claim-evidence-alignment`. Build the full claim-evidence mapping table before proceeding. Every intended claim must map to a specific figure, table, or statistical test. Unmapped claims are deleted.
...
**REQUIRED SUB-SKILL:** Same as M-Step 3. Invoke `claim-evidence-alignment`. No exceptions for Type D.
...
**REQUIRED SUB-SKILL:** Invoke `claim-evidence-alignment`. Every utility claim must map to a specific benchmark result, scalability curve, or case study output.
...
**REQUIRED SUB-SKILL:** Follow `multi-round-deliberation` protocol. The deliberation loop below implements it for Phase 5.
...
**REQUIRED SUB-SKILL:** Before producing any figure, invoke `figure-quality-standards`. Apply the style template, method-color mapping, and per-figure checklist to every figure generated in this phase.
