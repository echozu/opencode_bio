...
## 🔒 MANDATORY SELF-CHECK (Output at START of every response)

<IRON-LAW>
Before doing ANYTHING else in each response, you MUST output this status block:

```
<phase-status>
  current_phase: [0-6, or "not_started"]
  phase_name: [Phase name]
  last_completed_phase: [last completed, or "none"]
  session_state_read: [yes/no/not_needed]
  session_state_updated: [yes/no/will_update_at_end]
  hpc_detected: [yes/no/not_checked_yet]
  slurm_loaded: [yes/no/not_needed]
  gate_status: [which gate is next]
  analysis_type: [A/M/R/C/unknown]
</phase-status>
```

Rules:
- If `current_phase` ≠ `last_completed_phase + 1`, STOP and explain why you are skipping phases
- If `session_state_read` = "no" and `current_phase` = 0, you MUST read session-state.yaml first
- If `hpc_detected` = "not_checked_yet" and `current_phase` = 0, run HPC detection FIRST
- If `slurm_loaded` = "no" and `hpc_detected` = "yes", STOP and load slurm-execution skill NOW

At the END of your response, after completing the phase, update and output:
```
<phase-exit>
  completed_phase: [N]
  session_state_updated: [yes/no — if no, explain why]
  key_outputs: [list files produced]
  next_phase: [N+1]
  gate_required: [yes/no — which gate]
  waiting_for: "user confirmation to proceed to Phase [N+1]"
</phase-exit>
```
</IRON-LAW>
...
## ⚠️ MANDATORY: Infrastructure Skill Loading

<IRON-LAW>
In Phase 0, you MUST run environment detection BEFORE anything else:

1. `which sbatch 2>/dev/null && echo "HPC_DETECTED=true" || echo "HPC_DETECTED=false"`
2. `test -f hpc-env.yaml && echo "HPC_CONFIG=true" || echo "HPC_CONFIG=false"`
3. `cat hpc-env.yaml 2>/dev/null | head -5 || echo "NO_HPC_CONFIG"`

IF EITHER HPC_DETECTED=true OR HPC_CONFIG=true:
→ You MUST read the `slurm-execution` skill IMMEDIATELY — no exceptions
→ ALL heavy computation from Phase 4 onward MUST go through sbatch
→ NEVER run fastp, DADA2, MEGAHIT, MetaPhlAn, Kraken2, bowtie2, STAR, or any bioinformatics pipeline tool directly on the login node

This is a SAFETY issue. Running heavy tasks on login nodes:
- Gets your process killed by cluster admins
- Harms other users' work on the shared login node
- Wastes time when the killed job must be resubmitted
</IRON-LAW>

<EXTREMELY-IMPORTANT>
If you detect ANY sign of HPC environment (hpc-env.yaml exists, sbatch available,
user mentions cluster/server/HPC/服务器/集群), you MUST read `slurm-execution` skill.
This check happens ONCE in Phase 0 and the result persists for all subsequent phases.
</EXTREMELY-IMPORTANT>
